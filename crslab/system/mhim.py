...
class MHIMSystem(BaseSystem):
    ...
    def rec_evaluate(self, rec_predict, item_label):
        ...
        rec_predict  = rec_predict[:, self.item_ids]    # 只保留在 self.item_ids 中的分数
        _, rec_ranks = torch.topk(rec_predict, 100, dim=-1)    # top 100
        rec_ranks    = rec_ranks.tolist()
        item_label   = item_label.tolist()  # 把物品标签从 tensor 转换为列表，进行迭代
        # print(rec_ranks, item_label)
        for rec_rank, label in zip(rec_ranks, item_label):
            label = self.item_ids.index(label)  # 真实标签映射到物品列表中的索引位置
    ...
    def conv_evaluate(self, prediction, response, batch_user_id=None, batch_conv_id=None):
        ...
        if batch_user_id is None:
            for p, r in zip(prediction, response):  # 遍历预测响应、真实响应
                p_str = ind2txt(p, self.ind2tok, self.end_token_idx)    # 把预测响应序列转换为可读文本字符串
                r_str = ind2txt(r, self.ind2tok, self.end_token_idx)    # 转换真实响应
                self.evaluator.gen_evaluate(p_str, [r_str], p)  # 计算相似度等
        else:
    ...
    def train_recommender(self):
        self.init_optim(self.rec_optim_opt, self.model.parameters())    # 初始化优化器

        for epoch in range(self.rec_epoch):
            self.evaluator.reset_metrics()  # 重置评估器指标，保证每轮训练独立
