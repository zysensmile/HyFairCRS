...
class StandardEvaluator(BaseEvaluator):
    ...
    def __init__(self, save=False):
        ...
        if save:
            ...
            # self.root_file_path = f"/home/zhengyongsen/xzx/DCHL/MHIM/rank/{current_time_str}/"
            self.root_file_path = f"/home/zhengyongsen/xzx/DCHL/MHIM/rank/HTGReDial-MHIM/"
            os.makedirs(self.root_file_path, exist_ok=True)
        else:
    ...
    def report(self, epoch=-1, mode='test'):
        if self.root_file_path is not None and len(self.rankfile) > 0 and mode != 'train':
            file_path = os.path.join(self.root_file_path, f'-{epoch}')
