...
def run_crslab(config, save_data=False, restore_data=False, save_system=False, restore_system=False,
    ...
    if isinstance(config['tokenize'], str):
        ...
        train_dataloader = get_dataloader(config, CRS_dataset.train_data, CRS_dataset.train_review, vocab, CRS_dataset.review_id2entity)
        valid_dataloader = get_dataloader(config, CRS_dataset.valid_data, CRS_dataset.valid_review, vocab, CRS_dataset.review_id2entity)
        test_dataloader  = get_dataloader(config, CRS_dataset.test_data, CRS_dataset.test_review, vocab, CRS_dataset.review_id2entity)
    else:
        tokenized_dataset = {}
        train_dataloader  = {}
        valid_dataloader  = {}
