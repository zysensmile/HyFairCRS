...
class GraphDataset(torch.utils.data.IterableDataset):
    ...
    def trace_to_pyg_graph(self, seed, trace):
        vertices = set(trace.tolist())
        vertices.add(seed)
        vertices.remove(-1)
        sub_graph: DGLGraph = self.graph.subgraph(list(vertices))

        # mapping up_id to final_id
        node_up_id = sub_graph.ndata[dgl.NID].tolist()
        node_final_id = list()
        ...
        edge_type = self.graph.edata['edge_type'][sub_graph.edata[dgl.EID]].long()
